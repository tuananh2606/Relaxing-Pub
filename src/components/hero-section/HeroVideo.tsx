...
const HeroVideo = ({ items }: IHeroVideo) => {
  ...
  const [isOpenProviderModal, setIsOpenProviderModal] = useState<boolean>(false);
  ...
  const runtime =
    // @ts-ignore
    Number((details as IMovieDetail)?.runtime) ?? Number((details as ITvShowDetail)?.episode_run_time[0]);
  // const posterPath = item?.poster_path ? TMDB?.posterUrl(details?.poster_path || '', 'w342') : undefined;
    ...
    <div className="relative h-[56.25vw] max-h-[100dvh]">
      ...
      <div className="absolute bottom-[10%] left-[5%] flex w-[60%] flex-col justify-end md:w-[36%]">
        ...
        <div className="actions-btn mt-[1.5vw] flex items-center">
          ...
          <button
            onClick={openProviderModalHandle}
            className="mr-3 flex items-center justify-center rounded-[4px] bg-white px-2 py-2 transition hover:bg-[rgba(255,255,255,0.75)] md:px-7"
          >
            <span>
              <svg xmlns="http://www.w3.org/2000/svg" width="32" height="32" viewBox="0 0 24 24" fill="none">
      ...
      {isOpenProviderModal && (
        <div className="absolute left-0 top-0 z-40 flex h-full w-full justify-center">
          <SelectProvidersDialog
            ...
            setIsOpenProviderModal={setIsOpenProviderModal}
            setIsPlaying={setIsPlaying}
            type={type}
            title={title}
            origTitle={orgTitle}
