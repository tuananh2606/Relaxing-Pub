...
export const MediaDetails = (props: IMediaDetails) => {
  ...
  const directors =
    credits &&
    credits.crew &&
    credits.crew.filter(({ job }) => (params.type === 'movie' ? job === 'Director' : job === 'Executive Producer'));
              ...
              <div className="item z-10 flex flex-col grid-in-info">
                <div className="flex flex-col gap-2 md:flex-row">
                  ...
                  >
                    ...
                    <span className="ml-1">
                      {(mediaDetails?.vote_average as number) > 0 ? mediaDetails?.vote_average : 'NR'}
                    </span>
                  </div>
                  <div
                    className="w-fit rounded-2xl px-4 py-2"
                  ...
                  >
                    ...
                    {runtime ? <span> ‧ {ConvertToHourAndMinutes(runtime)}</span> : null}
                    {episodes ? (
                      <>
                    ...
                    <div className="flex">
                      ...
                      <div className="basis-2/3 flex-wrap">
                        {(mediaDetails as ITvShowDetail)?.created_by!.map((creator) => (
                          <span key={`creator-item-${creator.id}`} className="comma text-foreground">
                            {creator.name}
                          </span>
                        ))}
                      </div>
