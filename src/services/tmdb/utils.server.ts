...
export class TMDB {
  ...
  static similarUrl = (type: 'movie' | 'tv', id: number, page?: number, language?: string): string => {
  ...
  static recommendationUrl = (type: 'movie' | 'tv', id: number, page?: number, language?: string): string => {
...
export const postFetchDataHandler = (data: any, mediaType?: 'movie' | 'tv' | 'people'): IMedia[] => {
  const result: IMedia[] = [];
  ...
  const transform = (item: any): IMedia =>
    mediaType && mediaType !== 'people'
      ? {
          ...
          backdropPath: item.backdrop_path ? TMDB.backdropUrl(item.backdrop_path, 'w1280') : undefined,
